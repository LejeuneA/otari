...
@pytest.mark.asyncio
async def test_a_failed_build_is_not_kept(monkeypatch: pytest.MonkeyPatch) -> None:
    attempts = 0
...
@pytest.mark.asyncio
async def test_a_build_that_outlives_its_timeout_still_lands_in_the_registry(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
async def test_an_abandoned_local_check_keeps_its_worker_until_the_thread_finishes(
    ...
    Freeing the worker when the caller gives up would put a second call through the
...
def _worker_threads() -> set[threading.Thread]:
    """The threads a serialized guardrail owns, by the prefix the runner names them."""
    return {t for t in threading.enumerate() if t.name.startswith("guardrail-")}
...
async def test_evicting_a_local_guardrail_lets_its_worker_thread_exit(
    ...
    runner.evict("prompt-injection")
    # A turn of the loop first. The entry is still reachable from the callbacks
    # asyncio scheduled for the await above, which the loop drops on its next
    # pass; only then is the executor unreferenced and its thread woken to exit.
...
@pytest.mark.asyncio
async def test_the_registry_holds_no_more_entries_than_profiles_seen(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """What bounds the registry: a key is kept only while some profile resolves to it."""
    _install(monkeypatch)
    runner = GuardrailRunner()
...
async def test_an_eviction_during_a_build_still_leaves_both_waiters_one_worker(
    ...
    Handing each waiter a worker of its own would put two calls through one model
    object at once, which is the overlap the worker exists to prevent.
    """
    ...
    runner.evict("prompt-injection")  # so the finished build is never kept
