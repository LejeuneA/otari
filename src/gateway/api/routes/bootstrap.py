...
# Each surface is declared by the route group that serves it.
...
def _surfaces_hosted_on(kind: DeploymentKind) -> tuple[str, ...]:
    """The names of the surfaces this kind of deployment hosts, sorted."""
    return tuple(sorted(surface.name for surface in _DECLARED_SURFACES if kind in surface.deployments))


STANDALONE_SURFACES: tuple[str, ...] = _surfaces_hosted_on(DeploymentKind.STANDALONE)
HOSTED_SURFACES: tuple[str, ...] = _surfaces_hosted_on(DeploymentKind.HOSTED)
